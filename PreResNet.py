...
class sig_t(nn.Module):
    def __init__(self, num_classes, init=2):
        ...
        self.register_parameter(
            name="w", param=nn.parameter.Parameter(-init * torch.ones(num_classes, num_classes))
        )
...
class ResNet(nn.Module):
    ...
    def forward(self, x, lin=0, lout=5, eval=False):
        out = x
        if lin < 1 and lout > -1:
            out = self.conv1(out)
            out = self.bn1(out)
        ...
        if lout > 4:
            ...
            # proj = self.proj(feat)
            # tildey = self.transition(torch.cat((self.proj(feat), out), dim=1))
            tildey = self.transition(torch.cat((feat, out.detach()), dim=1))
            # tildey = self.transition(torch.cat((feat, out), dim=1))
        return out, tildey, None

    def feat_hook(self, module, inp, out):
        return F.avg_pool2d(out).flatten()
