...
class red_dataset(Dataset):
    def __init__(self, root_dir, transform, r, color="red", pred=[], mode="train"):
...
class red_dataloader:
    ...
    def run(self, mode, pred=[]):
        if mode == "warmup":
            all_dataset = red_dataset(
                r=self.r, root_dir=self.root_dir, transform=self.transform_train, mode="all"
            )
